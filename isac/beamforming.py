...
if __name__ == "__main__":
    ...
    # --- Directed AN ---
    R_N_dir = an_covariance_directed(H_K, g_e_hat, N_t, P_t, phi, K)
    print(f"\n[Directed AN]")
    print(f"tr(R_N)       : {np.real(np.trace(R_N_dir)):.4f}  "
          f"(expected {(1-phi)*P_t:.4f})")
    print(f"rank(R_N)     : {np.linalg.matrix_rank(R_N_dir)}  "
          f"(expected 1)")
    print(f"AN at CU 0    : {np.real(H_K[:,0].conj()@R_N_dir@H_K[:,0]):.2e}  "
          f"(should be ~0)")
    print(f"AN at CU 1    : {np.real(H_K[:,1].conj()@R_N_dir@H_K[:,1]):.2e}  "
          f"(should be ~0)")
    print(f"Is Hermitian  : {np.allclose(R_N_dir, R_N_dir.conj().T)}")

    # --- AN power toward Eve ---
    print(f"\n[AN power toward Eve]")
    print(f"Isotropic: {np.real(g_e_hat.conj()@R_N@g_e_hat):.6f}")
    print(f"Directed:  {np.real(g_e_hat.conj()@R_N_dir@g_e_hat):.6f}")
    ratio = (np.real(g_e_hat.conj()@R_N_dir@g_e_hat) /
             max(np.real(g_e_hat.conj()@R_N@g_e_hat), 1e-12))
    print(f"Directed is {ratio:.2f}x stronger toward Eve")

    # --- Power budget ---
    print(f"\n[Power budget]")
    print(f"ZF + isotropic: "
          f"{np.real(np.trace(W@W.conj().T))+np.real(np.trace(R_N)):.4f}  "
          f"(expected {P_t:.4f})")
    print(f"ZF + directed:  "
          f"{np.real(np.trace(W@W.conj().T))+np.real(np.trace(R_N_dir)):.4f}  "
          f"(expected {P_t:.4f})")
